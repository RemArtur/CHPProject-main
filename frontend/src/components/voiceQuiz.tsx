...
const QuestionVoice: React.FC = () => {
  ...
  const [isLastRespondent, setIsLastRespondent] = useState(false); // Добавляем состояние для последнего респондента

  const handleNextQuestion = async () => {
    if (
      response.trim() !== "" ||
      selectedOption ||
      currentQuestion?.answer_type === "desc"
    ) {
      if (currentQuestion?.question === "Скажите идентификатор вопроса") {
        ...
        setCurrentQuestion({
          question: "Вы последний опрашиваемый по данному идентификатору?",
          answer_type: "test",
          options: {
            да: { title: "Да", redirect_question: "2" },
            нет: { title: "Нет", redirect_question: "2" },
          },
        });
        return;
      } else if (
        currentQuestion?.question ===
        "Вы последний опрашиваемый по данному идентификатору?"
      ) {
        // Обрабатываем ответ на вопрос о последнем участнике
        setIsLastRespondent(selectedOption === "да");
        setResponse("");
        setSelectedOption(null);

        // Отправляем информацию о том, является ли пользователь последним респондентом
        try {
          await axios.post(`${backendUrl}/set-last-respondent`, {
            is_last_respondent: selectedOption === "да",
          });
        } catch (error) {
          console.error(
            "Ошибка при отправке статуса последнего респондента:",
            error
          );
        }
        ...
        fetchNextQuestion(
          ...
          currentSheet,
          currentBlock,
          currentQuestion.next_question ?? "default_value" // Используем оператор нулевого слияния
        );
        return;
      }
      ...
      if (currentQuestion?.answer_type === "test-rec" && !showOptions) {
        ...
        try {
          const result = await axios.post<number>(
            `${backendUrl}/choose-option`,
            {
              options: currentQuestion.options,
              answer_text: response,
            }
          );
          console.info("Option chosen:", result.data);
          setIsLoading(false);
          if (result.data !== 0) {
              ...
              setTimeout(() => {
                if (textAreaRef.current) {
                  textAreaRef.current.scrollTop =
                    textAreaRef.current.scrollHeight; // Прокручиваем вниз
                }
              }, 0);
  ...
  const saveResponse = (resp: string, option?: string) => {
    if (currentQuestion) {
      const newResponse: Response = {
        ...
        answer_option:
          currentQuestion.answer_type === "test" ||
          currentQuestion.answer_type === "test-rec"
            ? option || undefined
            : undefined,
  ...
  const generateIshikawaDiagram = async () => {
    try {
      const formattedResponse = await axiosInstance.post("/format-answers", {
        questions: responses,
      });
      const formattedText = formattedResponse.data.formatted_text;

      console.log("creating ishikava");
      const isikava_text = await axiosInstance.post("/create-isikava", {
        formatted_text: formattedText,
      });
      setIshikawaText(isikava_text.data.isikava);

      // Request the image visualization
      const ishikawaImageResponse = await axiosInstance.post(
        "/generate-ishikawa-image",
        { formatted_text: isikava_text.data.isikava },
        { responseType: "blob" }
      );
  ...
  const generateTimeline = async () => {
    try {
      const filteredResponses = responses.filter(
        (response) => response.time_related
      );
      ...
      const timelineTextResponse = await axiosInstance.post(
        "/create-schedule",
        { formatted_text: formattedText }
      );
      ...
      // Request the image visualization
      const timelineImageResponse = await axiosInstance.post(
        "/generate-timeline-image",
        { formatted_text: timelineTextResponse.data.timeline },
        { responseType: "blob" }
      );
  ...
  const downloadDescriprion = async () => {
    ...
    const formattedResponse = await axiosInstance.post(
      `${backendUrl}/format-answers`,
      { questions: responses }
    );
    const formattedText = formattedResponse.data.formatted_text;
    ...
    const listAboutQuestions = responses
      .slice(3, 14)
  ...
  const downloadIshikawaDiagram = () => {
    ...
    const dataUri =
      "data:text/plain;charset=utf-8," + encodeURIComponent(IshikawaText);
  ...
  const downloadTimeline = () => {
    // Download the text file
    const dataUri =
      "data:text/plain;charset=utf-8," + encodeURIComponent(TimelineText);
  ...
  const downloadResponsesAsTxt = async () => {
    try {
      ...
      const response = await axios.post(`${backendUrl}/format-answers`, {
        questions: responses,
      });
  ...
  const processNextStep = (question: Question) => {
    ...
    if (question.next_question === "finish_all") {
      ...
      question.next_question === "finish_block" ||
      (currentBlockData &&
        currentBlockData.finish_question === currentQuestionNum)
    ) {
      handleEndOfBlock();
      return;
      ...
      const redirectIndex = responses.findIndex(
        (r) =>
      ...
      fetchNextQuestion(
        ...
        currentSheet,
        currentBlock,
        selectedOpt.redirect_question
      );
    } else if (selectedOpt?.redirect_branch) {
      const { branch, sheet, block_num, question_num } =
        selectedOpt.redirect_branch;
  ...
  const handleEndOfBlock = () => {
    const currentBlockIndex = blocks.findIndex(
      (block) => block.block_num === currentBlock
    );
  ...
  const fetchNextQuestion = async (
    ...
    try {
      const response = await axios.get<Question>(
        `${backendUrl}/get-question/${questionId}`
      );
      ...
      {!hasStarted ? (
          ...
          {isLastRespondent && (
            <>
              <button
                ...
                className={`p-4 rounded-xl shadow-sm shadow-blue-800 w-full text-center mt-4 ${
                  !isIshikawaGenerated
        ...
        <>
          ...
          <div className="flex flex-col gap-4">
            ...
            (currentQuestion.answer_type === "test-rec" && !showOptions) ? (
              <textarea
              ...
              <div className="flex flex-col gap-2" style={{ fontSize: 25 }}>
                {currentQuestion.options &&
                  Object.keys(currentQuestion.options)
                    .filter(
                      (key) => currentQuestion.options![key].title !== "empty"
                    ) // Фильтрация опций с title === 'empty'
                    .map((key) => (
                      <label key={key} className="flex items-center">
                        ...
                        <span
                          className={`mr-2 font-bold ${
                            selectedOption === key ? "text-green-500" : ""
                          }`}
                        >
                          {key}.
                        </span>
          ...
          <div className="flex gap-4 items-center mt-10">
            <button
              ...
              } shadow-sm shadow-gray-800 p-2 rounded-xl w-full`}
            >
              Следующий вопрос
            </button>
            ...
            <button
              ...
              className={`p-2 rounded-full ${
                isSpeakingEnabled ? "bg-green-500" : "bg-gray-700"
              }`}
            >
              {isSpeakingEnabled ? "🔊" : "🔇"}
            </button>
